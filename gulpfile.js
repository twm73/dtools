...
/**
 * Updates the extension version in defaultConfig.
 * @returns {*}
 */
function updateVersionInExtensionConfig() {
    return gulp.src(extensionJSON.projectPaths.configPath + '/' + extensionJSON.configFileName)
        .pipe(map(function (file, cb) {
            const extensionJson = JSON.parse(fs.readFileSync(extensionJSON.panelFolder + 'extension.json', "utf-8"));
            let fileContents = file.contents.toString();
            fileContents = fileContents.replace(/"appVersion": "\d+\.\d+\.\d+"/m, 'appVersion: "' + projectJSON.version + '"');
            fileContents = fileContents.replace(/("major": )(\d+)/mg, "$1" + extensionJson.extensionVersion[0]);
            fileContents = fileContents.replace(/("minor": )(\d+)/mg, "$1" + extensionJson.extensionVersion[1]);
            fileContents = fileContents.replace(/("patch": )(\d+)/mg, "$1" + extensionJson.extensionVersion[2]);
...
function makeZxp(done) {
    let zxpStrings = [];
    let fileName = extensionJSON.extensionName + ' ' + extensionJSON.extensionVersion.join('.');
